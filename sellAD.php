...
mysqli_select_db($con ,'login');
      ...
      if (!empty($_POST['nomproduit']) && !empty($_POST['prixproduit']) && !empty($_POST['quantproduit']) && !empty($_POST['descproduit']) && !empty($_POST['catégorie']) && !empty(($_POST['catégorie']))) {
        ...
        $catégorie =$_POST['catégorie'];
            ...
            $addpro = "INSERT INTO `produit` (`nom`, `img`, `prix`, `quantite`, `description`,`categorie`) VALUES ('$nomproduit', '$imgproduit', '$prixproduit', '$quantproduit','$descproduit','$catégorie')";
            $rq = mysqli_query($con,$addpro);
          die("<p class='alert success'>Félicitations ! Votre nouveau produit a été ajouter avec succès !</p><br><center><a href='sell.php'>Ajouter un autre produit</a> - <a href='home.php'>Voir tous les produit</a></center>");
      }else{
          /*exit ("<p class='alert error'><b>Attention !</b> Verifier que les coordonnées saisie correcte...</p>");
    */  }
